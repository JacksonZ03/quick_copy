...
check_sd_card() {
    if [ ! -d "$SOURCE_DIR" ]; then
        echo "SD card not detected. Please insert the SD card and press Enter to continue."
        read -r
        if [ ! -d "$SOURCE_DIR" ]; then
            echo "SD card still not detected. Exiting."
            exit 1
        fi
    fi
}

# Check for SD card before proceeding
check_sd_card
